...
local function ease(val, target)
  ...
  function check_conveyor_chain_player()
    ...
    while true do
      ...
      if is_conveyor(tile) then
        ...
        -- same user move_id, but store a snapshot so that 
        -- stepping backward will undo this tile
        push_snapshot()
        target_px, target_py = nx, ny
        moving = true
        return
  ...
  function check_rock_conveyor(r)
    ...
    local nx, ny = r.x + dx, r.y + dy  -- Rock's proposed new pixel position
  
    -- Compute tile coordinates for the rock's target.
    local rock_target_tile_x = flr(nx/8)
    local rock_target_tile_y = flr(ny/8)
  
    -- Get player's tile coordinate (using target if already moving)
    local player_tile_x = moving and flr(target_px/8) or flr(px/8)
    local player_tile_y = moving and flr(target_py/8) or flr(py/8)
  
    -- If the rock's target tile is occupied by the player...
    if rock_target_tile_x == player_tile_x and rock_target_tile_y == player_tile_y then
      -- Compute the tile the player would move into (one tile further in the same direction)
      local p_nx = (player_tile_x + dx/8) * 8
      local p_ny = (player_tile_y + dy/8) * 8
      -- Only push the player if that tile is free
      if not collide(p_nx, p_ny, false, nil) then
        push_snapshot()
        target_px, target_py = p_nx, p_ny
        moving = true
      else
        -- Cannot push the player: abort rock movement.
        return
      end
    end
  
    -- If some other collision (wall, rock, etc.) blocks the rock, abort.
